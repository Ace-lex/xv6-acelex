...
int e1000_transmit(struct mbuf *m) {
  ...
  if (tx_ring[tx_ring_idx].status != E1000_TXD_STAT_DD) {
    printf("e1000_transmit error\n");
    release(&e1000_lock);
    return -1;
  }

  // 释放之前保存的mbuf
  if (tx_ring[tx_ring_idx].addr && tx_ring[tx_ring_idx].length) {
    mbuffree(tx_mbufs[tx_ring_idx]);
    tx_ring[tx_ring_idx].addr = 0;
    tx_ring[tx_ring_idx].length = 0;
  }
...
static void e1000_recv(void) {
  ...
  // 循环的原因是为了应对大量数据包到来的情况，提高效率
  while (1) {
    int rx_ring_idx = regs[E1000_RDT];
